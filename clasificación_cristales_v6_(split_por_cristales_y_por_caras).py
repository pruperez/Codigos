# -*- coding: utf-8 -*-
"""Clasificación Cristales v6 (split por cristales y por caras).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wIwNIc-5-AiEYrbNbwq7Vq_AfneQKIJU
...
# ─────────────────────────────────────────────
# 1. DIVIDIR DATOS EN TRAIN / VAL POR CRISTALES
# ─────────────────────────────────────────────
def auto_split(raw_dir, train_dir, val_dir, val_ratio=0.2):
    """
    Divide los datos en train y val por cristal, no por imagen.
    Todas las imágenes del mismo cristal van al mismo split.
    """
    if Path(train_dir).exists():
        print("[INFO] Las carpetas train/val ya existen, saltando split.")
        return

    print(f"[INFO] Dividiendo datos por cristal ({int((1-val_ratio)*100)}% train / {int(val_ratio*100)}% val)...")

    for clase in CLASES:
        carpeta_clase = Path(raw_dir) / clase
        archivos = [f for f in carpeta_clase.glob("*")
                    if f.suffix.lower() in (".png", ".jpg", ".jpeg", ".tif", ".bmp")]

        # Agrupar archivos por número de cristal
        # Nombre: Foto_numcristal_cara_iter_numiter.jpg
        cristales = {}
        for f in archivos:
            partes = f.stem.split("_")  # ["Foto", "93", "a", "iter", "2"]
            num_cristal = partes[1] #+ "_" + partes[2] (descomentar si dividimos por caras de cristal)     # "93"
            if num_cristal not in cristales:
                cristales[num_cristal] = []
            cristales[num_cristal].append(f)

        # Dividir los cristales (no las imágenes) en train/val
        lista_cristales = list(cristales.keys())
        random.seed(CONFIG["seed"])
        random.shuffle(lista_cristales)

        n_val = int(len(lista_cristales) * val_ratio)
        cristales_val   = lista_cristales[:n_val]
        cristales_train = lista_cristales[n_val:]

        # Copiar todas las imágenes de cada cristal a su split
        for split, cristales_split in [("train", cristales_train), ("val", cristales_val)]:
            dest = Path(train_dir if split == "train" else val_dir) / clase
            dest.mkdir(parents=True, exist_ok=True)
            for num_cristal in cristales_split:
                for f in cristales[num_cristal]:
                    shutil.copy(f, dest / f.name)

        n_img_train = sum(len(cristales[c]) for c in cristales_train)
        n_img_val   = sum(len(cristales[c]) for c in cristales_val)
        print(f"  {clase}: {len(cristales_train)} cristales en train "
              f"({n_img_train} imágenes) | "
              f"{len(cristales_val)} cristales en val "
              f"({n_img_val} imágenes)")
...
def entrenar(config):
    ...
    loss_weights = torch.tensor([1.0, 2.0, 5.0], dtype=torch.float).to(DEVICE) #descomentar para pesos manuales
...
def medir_tiempo_inferencia(model, config, n_repeticiones=100):
    """
    Mide el tiempo medio de inferencia del modelo sobre una imagen.
    Se hacen n_repeticiones para obtener una media estable.
    """
    from PIL import Image
    import numpy as np
    ...
    # Crea una imagen aleatoria del mismo tamaño que las tuyas
    img_dummy = Image.fromarray(
        np.random.randint(0, 255, (800, 600, 3), dtype=np.uint8)
    )
    tensor = transform(img_dummy).unsqueeze(0).to(DEVICE)

    # Calentamiento (warm-up) — las primeras inferencias son más lentas
    with torch.no_grad():
        for _ in range(10):
            _ = model(tensor)
    ...
    print(f"\n[INFO] Tiempo de inferencia ({n_repeticiones} repeticiones):")
...
media_ms, std_ms = medir_tiempo_inferencia(model, CONFIG, n_repeticiones=100)
...
shutil.rmtree(CONFIG["val_dir"], ignore_errors=True)
print("Carpetas borradas correctamente")
